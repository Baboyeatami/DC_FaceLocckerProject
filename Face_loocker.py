...
def InsertCoin():
    ...
    while True:
        while (not Stop_thread2):
            ...
            GPIO.setup(channel_is_on, GPIO.IN) #elisi ang channel na variable sa port na gusto nimo
            #GPIO.output(channel_is_on,LOW)
            channel_is_on = GPIO.input(channel)  # Returns 0 if OFF or 1 if ON elisi ang channel na variable sa port na gusto nimo
